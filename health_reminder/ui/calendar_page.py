...
class CalendarPageMixin:
    ...
    def _daily_metrics(self, parent: tk.Frame) -> None:
        ...
        for index, (label, key, color) in enumerate(metrics):
            ...
            tk.Label(
                item, text=label, bg="#f9fafb", fg=MUTED, font=scaling.font("Microsoft YaHei UI", 9)
    ...
    def _month_calendar(self, parent: tk.Frame) -> None:
        ...
        for index, name in enumerate(["一", "二", "三", "四", "五", "六", "日"]):
            tk.Label(
                grid, text=name, bg="white", fg=MUTED, font=scaling.font("Microsoft YaHei UI", 9)
            ).grid(row=0, column=index, sticky="ew", pady=4)
    ...
    def _calendar_cell(
        ...
        if selected:
            color = "#111827"
        cell = tk.Frame(
            grid, bg=color, height=50, highlightbackground="#ffffff", highlightthickness=2
        )
    ...
    def _selected_day_detail(self, parent: tk.Frame) -> None:
        ...
        for index, (label, value, color) in enumerate(values):
            ...
            tk.Label(
                box, text=label, bg="white", fg=MUTED, font=scaling.font("Microsoft YaHei UI", 9)
            ).pack(anchor="w")
            tk.Label(
                box,
                text=value,
    ...
    def _history_switcher(self, parent: tk.Frame) -> None:
        ...
        ttk.Button(header, text="柱状统计图", command=lambda: self._set_history_mode("chart")).pack(
            side="right", padx=(8, 0)
        )
        ttk.Button(header, text="列表视图", command=lambda: self._set_history_mode("list")).pack(
            side="right"
        )
        body = tk.Frame(parent, bg="white")
    ...
    def _set_history_mode(self, mode: str) -> None:
        ...
        if (
            self.page == "calendar"
            and self._history_body is not None
            and self._history_body.winfo_exists()
        ):
            self._render_history_body()
    ...
    def _history_chart(self, parent: tk.Frame) -> None:
        ...
        max_value = max(
            1,
            max(
                metric_value(history[day], key)
                for day in days
                for key in ("water_count", "stand_count", "away_count")
            ),
        )
        ...
        for index, day in enumerate(days):
            ...
            canvas.create_text(
                ...
                text=day[-5:],
                fill=MUTED,
                font=scaling.font("Microsoft YaHei UI", 8),
            )
        ...
        for index, (label, color) in enumerate(legend):
            ...
            canvas.create_text(
                x + 36, 244, text=label, fill=MUTED, font=scaling.font("Microsoft YaHei UI", 9)
            )
