...
class PopupManager:
    ...
    def _show_simple_popup(self, event: ReminderEvent, on_dismiss: Callable[[str], None] | None = None) -> None:
        ...
        width, height = scaling.s(420), scaling.s(210)
        ...
        tk.Label(body, text=event.title, bg="white", fg="#1f2328",
                 ...
                 font=scaling.font("Microsoft YaHei UI", 16, "bold")).pack(anchor="w")

        tk.Label(

            body, text=event.message, bg="white", fg="#5c6570",

            font=scaling.font("Microsoft YaHei UI", 11), wraplength=340, justify="left",
    ...
    def show_away_reason(self, on_select: Callable[[str], None]) -> None:
        ...
        tk.Label(frame, text="刚才离开了一会儿",
                 ...
                 font=scaling.font("Microsoft YaHei UI", 15, "bold")).pack(anchor="w")

        tk.Label(frame, text="选择原因后会记入今日健康数据",

                 bg="white", fg="#68717d",

                 font=scaling.font("Microsoft YaHei UI", 9)).pack(anchor="w", pady=(5, 16))
