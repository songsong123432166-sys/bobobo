...
class SettingsPageMixin:
    ...
    def _build_settings(self, parent: tk.Frame) -> None:
        ...
        self._settings_group(
            ...
            "工作时间",
            [
                ("上班时间", "work_time.start", config["work_time"]["start"]),
                ("下班时间", "work_time.end", config["work_time"]["end"]),
            ],
        )
        self._settings_group(
            wrap,
            ...
            [
                (
                    "久坐间隔（分钟）",
                    "reminders.sedentary_interval_minutes",
                    config["reminders"]["sedentary_interval_minutes"],
                ),
                (
                    "喝水间隔（分钟）",
                    "reminders.water_interval_minutes",
                    config["reminders"]["water_interval_minutes"],
                ),
                (
                    "喝水稍后提醒（分钟）",
                    "reminders.water_snooze_minutes",
                    config["reminders"]["water_snooze_minutes"],
                ),
                (
                    "合并提醒窗口（分钟）",
                    "reminders.merge_window_minutes",
                    config["reminders"]["merge_window_minutes"],
                ),
            ],
        )
        self._settings_group(
        ...
        self._settings_group(
            ...
            [
                (
                    "离开判断时间（秒）",
                    "detection.away_after_seconds",
                    config["detection"]["away_after_seconds"],
                ),
                (
                    "空闲判断时间（秒）",
                    "detection.idle_after_seconds",
                    config["detection"]["idle_after_seconds"],
                ),
                (
                    "摄像头触发空闲时间（秒）",
                    "detection.camera_idle_threshold_seconds",
                    config["detection"]["camera_idle_threshold_seconds"],
                ),
                (
                    "摄像头检测间隔（秒）",
                    "detection.camera_interval_seconds",
                    config["detection"]["camera_interval_seconds"],
                ),
                (
                    "无人后摄像头检测间隔（秒）",
                    "detection.camera_away_interval_seconds",
                    config["detection"]["camera_away_interval_seconds"],
                ),
                (
                    "离席红灯阈值（秒）",
                    "detection.away_red_after_seconds",
                    config["detection"]["away_red_after_seconds"],
                ),
                (
                    "站起检测间隔（秒）",
                    "detection.stand_watch_interval_seconds",
                    config["detection"]["stand_watch_interval_seconds"],
                ),
                (
                    "站起检测持续时间（秒）",
                    "detection.stand_watch_duration_seconds",
                    config["detection"]["stand_watch_duration_seconds"],
                ),
                ("摄像头检测", "detection.camera_enabled", config["detection"]["camera_enabled"]),
                (
                    "隐私模式（暂停摄像头）",
                    "detection.privacy_mode",
                    config["detection"]["privacy_mode"],
                ),
                (
                    "中央弹窗",
                    "detection.center_popup_enabled",
                    config["detection"]["center_popup_enabled"],
                ),
            ],
        )
        ...
        self._settings_group(
            ...
            [
                ...
                (
                    "最长久坐目标（分钟）",
                    "goals.max_sit_streak_minutes",
                    config["goals"]["max_sit_streak_minutes"],
                ),
            ],
        )
        self._settings_group(
            ...
            [
                ...
                (
                    "提示音音量（0-100）",
                    "system.sound_volume_percent",
                    config["system"]["sound_volume_percent"],
                ),
                (
                    "弹窗留存时长（秒）",
                    "system.popup_retention_seconds",
                    config["system"]["popup_retention_seconds"],
                ),
            ],
        )
        ...
        ttk.Button(wrap, text="保存设置", command=self._save_settings).pack(
            anchor="e", pady=(0, 18), padx=2
        )
        self._how_it_works_section(wrap)
        self._settings_log(wrap)

    def _settings_group(
        self, parent: tk.Frame, title: str, rows: list[tuple[str, str, Any]]
    ) -> None:
        ...
        for label, key, value in rows:
            ...
            tk.Label(
                row, text=label, bg="white", fg=TEXT, font=scaling.font("Microsoft YaHei UI", 10)
            ).pack(side="left")
    ...
    def _popup_mode_toggle(self, parent: tk.Frame) -> None:
        ...
        tk.Label(
            left,
            text="开启后使用 Windows 原生通知，兼容多屏高 DPI",
            bg="white",
            fg=MUTED,
        ...
        self._popup_mode_label = tk.Label(
            ...
            bg="white",
            fg=BLUE if current == "system" else MUTED,
    ...
    def _preset_tools(self, parent: tk.Frame) -> None:
        ...
        for text, values in presets:
            ttk.Button(row, text=text, command=lambda item=values: self._apply_preset(item)).pack(
                side="left", padx=(0, 8)
            )
    ...
    def _test_tools(self, parent: tk.Frame) -> None:
        ...
        row = tk.Frame(card, bg="white")
        row.pack(fill="x", padx=18, pady=(0, 16))
        ttk.Button(row, text="测试提示音", command=self.on_test_sound).pack(
            side="left", padx=(0, 8)
        )
        ttk.Button(row, text="测试右下角弹窗", command=self.on_test_popup).pack(
            side="left", padx=(0, 8)
        )
        ttk.Button(row, text="测试中央弹窗", command=self.on_test_center_popup).pack(
            side="left", padx=(0, 8)
        )
