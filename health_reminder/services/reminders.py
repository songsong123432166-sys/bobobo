...
class ReminderDecisionEngine:
    ...
    def due(
        self, now: datetime, config: dict[str, Any], paused: bool = False
    ) -> ReminderEvent | None:
        """检查待触发的提醒。"""
        ...
        if (sedentary_due and water_due) or (close and (sedentary_due or water_due)):
            ...
            return ReminderEvent(
                "combined", REMINDER_TITLES["combined"], ("休息一下，喝口水，顺便活动肩膀。")
            )
        if sedentary_due:
            self.sedentary_alert_active = True
            return ReminderEvent(
                "sedentary", REMINDER_TITLES["sedentary"], ("离开椅子走动两分钟，眼睛也休息一下。")
            )
        if water_due:
            self.last_water = now
            self.water_snooze_until = None
            return ReminderEvent(
                "water", REMINDER_TITLES["water"], ("补充一杯水，给今天的健康分加一点蓝色。")
            )
        return None
    ...
    def remaining_seconds(self, now: datetime, config: dict[str, Any]) -> tuple[int, int]:
        """返回距下次提醒的剩余秒数。"""
        reminders = config.get("reminders", {})
        sedentary_due_at = self.last_sedentary + timedelta(
            minutes=int(reminders.get("sedentary_interval_minutes", 45))
        )
        water_due_at = self.last_water + timedelta(
            minutes=int(reminders.get("water_interval_minutes", 60))
        )
        if self.water_snooze_until is not None:
            water_due_at = max(water_due_at, self.water_snooze_until)
        return (
...
class ReminderService:
    ...
    def _check_work_events(self, now: datetime, config: dict[str, Any]) -> None:
        ...
        if is_time_between(
            now.time(),
            start_t,
            (datetime.combine(now.date(), start_t) + timedelta(minutes=2)).time(),
        ):
            event_kind = "work_start"
        elif is_time_between(
            now.time(), end_t, (datetime.combine(now.date(), end_t) + timedelta(minutes=2)).time()
        ):
            event_kind = "work_end"
    ...
    def _update_presence_from_input(
        self, now_mono: float, idle_seconds: float, config: dict[str, Any]
    ) -> None:
    ...
    def camera_diagnostic(self) -> str:
        ...
        status = (
            "检测到有人"
            if result.person_present
            else "未检测到人" if result.person_present is False else "无法判断"
        )
        return f"{status}\n可用：{result.available}\n结果：{result.message}"
