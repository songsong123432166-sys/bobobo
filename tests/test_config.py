...
class ConfigStoreTest(unittest.TestCase):
    def test_load_merges_defaults(self):
        with TemporaryDirectory() as temp:
            ...
            self.assertEqual(config["detection"]["away_to_stand_max_seconds"], 1200)
            self.assertEqual(config["detection"]["away_to_stand_min_sedentary_seconds"], 300)
    ...
    def test_version_is_migrated_to_current_release(self):
        with TemporaryDirectory() as temp:
            ...
            config = ConfigStore(paths).load()
            self.assertEqual(config["version"], "v2.8.1")
